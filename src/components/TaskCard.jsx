...
import { BarChart3, Check, ChevronDown, Clock, Pencil, Trash2 } from 'lucide-react'
...
export default function TaskCard({ task, now }) {
  const { role, isDone, toggleDone, setEditor, setDetailTaskId, progressByTask, students, deleteTask } = useApp()
    ...
    <article className={cx('rounded-xl border border-line bg-surface transition-colors', done && 'bg-surface/60')}>
      <div className="flex gap-3 p-4">
            ...
            <div className="mt-3 border-t border-dashed border-line pt-3">
              ...
              {isKetua && (
                <div className="mt-3 flex flex-wrap gap-2">
                  <Button size="sm" variant="outline" onClick={() => setDetailTaskId(task.id)}><BarChart3 className="h-4 w-4" />Lihat progres</Button>
                  <Button size="sm" variant="soft" onClick={() => setEditor({ task })}><Pencil className="h-4 w-4" />Ubah</Button>
                  <Button size="sm" variant="danger" onClick={onDelete}><Trash2 className="h-4 w-4" />Hapus</Button>
                </div>
              )}
            </div>
          )}
        ...
        <button
          ...
          className="-mr-1 h-8 w-8 shrink-0 self-start rounded-lg text-muted hover:bg-sunken"
        >
          <ChevronDown className={cx('mx-auto h-5 w-5 transition-transform', open && 'rotate-180')} />
        </button>
      </div>
    </article>
  )
